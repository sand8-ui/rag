...
def add_retrieval_pad(doc: Document, title: str, sections: list[str]) -> None:
    add_heading(doc, "检索补充与抽检", 1)
    for round_no in range(1, 4):
        for index, heading in enumerate(sections, start=1):
            add_paras(
                doc,
                [
                    (
                        f"抽检问答 {round_no}-{index}（{title} / {heading}）。"
                        f"用户口语问到「{heading}」时，只引用该节已写规则：先一句结论，再补条件与例外。"
                        "需要订单号、酒店全称、入住日期、证件姓名再给个性化处理。"
                        "禁止改写取消阶梯、房价、房型人数、押金区间等已在别的文档出现的数字。"
                        "演示环境没有收银台、积分账户、开票接口、改姓名接口；删除订单不可恢复。"
                        "人身安全走紧急升级文档；宠物、发票、支付、改期以各专项为准，本节不另造数字。"
                    )
                ],
            )
...
def doc_change_dates() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "404：订单不属于当前用户，或已被删除。",
            "改期不改房价、不改房型。入住当天改期可能被现场视为占用库存，客服应提示需酒店再确认。不可取消产品原则上不能改期，坚持则转人工。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 订单改期作业手册",
        ["1. 允许改什么", "2. 次数与费用", "3. 与搜索条件的关系", "4. 失败场景"],
    )
...
def doc_invoice() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "不可开「含积分的全额房费票」。",
            "演示没有自动开票接口。建议离店后 30 日内申请，超 90 日需财务审批。电子票约 3 个工作日。预付由平台开代订/服务费票，到店现付才可能是酒店住宿票。押金迷你吧 SPA 现场开。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 发票与报销指引",
        ["1. 谁来开票", "2. 需要用户提供的字段", "3. 时限", "4. 积分与优惠"],
    )
...
def doc_payment() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "下单不经过支付网关，问「卡被刷了吗」应回答演示不会真实扣款。上线后才有预付、到店付、信用卡担保三模式。退款原路，信用卡 1–15 工作日。删除订单不会触发退款流水。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 支付、担保与退款路径",
        ["1. 当前演示", "2. 上线后的三种模式", "3. 退款路径", "4. 外币"],
    )
...
def doc_pets() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "上海外滩与成都博舍、北京王府井原则上不接受伴侣宠物，服务犬除外。杭州西溪小型犬可能接受，须提前 48 小时，清洁费约 300–600 元/晚现场付。禁止把猫装包混入外滩店。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 宠物与过敏协办手册",
        ["1. 四家店宠物政策（演示口径）", "2. 申请要写清", "3. 过敏客人"],
    )
...
def doc_dining() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "演示房价默认不含早餐。时段常见 6:30–10:00。迷你吧按酒水单从押金扣。客房送餐通常 6:00–23:00 另加服务费。禁止外带火锅烧烤进房加热。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 早餐、餐饮与迷你吧说明",
        ["1. 早餐", "2. 餐厅预约", "3. 迷你吧与客房送餐", "4. 宗教与饮食禁忌"],
    )
...
def doc_safety() -> None:
    ...
    add_paras(
        ...
        [
            "公共区域可能有监控。客房、卫生间无监控。",
            "不要向第三方泄露用户证件号与订单手机号。",
            "房间保险箱仅便利，大额与护照建议前台保管。StayWise 不赔付失窃。滑倒泳池交通先救人再报安保，禁止聊天做医疗诊断或承诺医疗赔偿金额。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 安全、贵重物品与保险须知",
        ["1. 贵重物品", "2. 消防与逃生", "3. 意外", "4. 监控与隐私"],
    )
...
def doc_business() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "5 间及以上视为团队，取消窗口常比散客更早。演示按散客单处理。协议价一律转人工，禁止用散客演示价冒充。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 商务出行、会议与企业协议",
        ["1. 会议与宴会", "2. 连住与团队房", "3. 出行单与对公结算"],
    )
...
def doc_housekeeping() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "清洁大约 9:00–15:00，挂请勿打扰则跳过。续住改期延后离店不保证原房号，房价沿用快照。遗失物品保管常见 30–90 天，运费到付，不能强制酒店承认一定找到。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 客房清洁、续住与物品遗失",
        ["1. 清洁时段", "2. 续住", "3. 遗失物品", "4. 损坏"],
    )
...
def doc_transfer_parking() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "演示订单不含免费接机。价格必须问酒店礼宾。上海虹桥通常比浦东便宜。杭州司机常不能开到别墅门口。成都先问双流还是天府。北京活动日可能路口下客。外滩过夜停车约 150–200 元量级以现场为准。不承诺空闲充电桩。",
        ],
    )
    add_retrieval_pad(
        doc,
...
def doc_upgrades() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "无羽绒、乳胶替换：尽量满足，数量有限。",
            "免费升房视房态由前台决定，会员只增加优先级不是权利。最高档无法再升。付费升级差价现场付，平台订单价不改。布置代转达不代收，外滩户外大型布置可能被拦。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 升房、特殊布置与迎宾",
        ["1. 免费升房", "2. 付费升级", "3. 生日、求婚、纪念日", "4. 枕头被褥与气味"],
    )
...
def doc_fraud() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "AI 禁止帮用户「试试再转一笔追回」。",
            "StayWise 不要个人微信支付宝比特币、不要密码验证码 CVV。重复扣款先转 1 元验证、短链改密、酒店改收款账户都是诈骗。已转账先联系银行，平台不垫付。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 防诈骗与假客服识别",
        ["1. 真假辨别", "2. 常见剧本", "3. 已经转了钱"],
    )
...
def doc_visa_letter() -> None:
    ...
    add_paras(
        ...
        [
            ...
            "订单截图不是盖章确认函。演示不能生成公章 PDF、不能出具邀请函、不能担保入境。改期后旧函作废。不可取消更易被使馆采信，但不得怂恿用户买不可退而不说明风险。",
        ],
    )
    add_retrieval_pad(
        doc,
        "StayWise 签证确认函与英文证明",
        ["1. 演示能给什么", "2. 上线后规划", "3. 姓名必须对护照"],
    )
